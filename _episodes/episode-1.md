...
브리지 콜 참가자 목록에 이름이 하나둘 올라왔다. 그 주 온콜이었던 SRE 한유리는 이미 들어와 있었다. 알림이 울린 지 41초 만에 페이지를 ack했다고 타임라인에 찍혀 있었다. 새벽 두 시에 41초라니. 지안은 그 숫자를 머릿속 어딘가에 접어두었다.

"인시던트 커맨더 제가 잡겠습니다." 지안이 말했다. "유리 씨, 지금 보이는 거 뭐예요?"

"payments-db 커넥션 풀이 꽉 찼어요. 액티브 커넥션 500에 대기 큐가 계속 쌓이고 있고요. API 파드는 다 살아 있는데 DB를 못 잡아서 타임아웃 나는 거예요."

"뭔가 커넥션을 먹고 있다는 건데. 배포 있었어요?"

"결제팀 마지막 배포는 어제 오후 네 시예요." 도현의 목소리에 골목 바람 소리가 섞여 들어왔다.

"배포가 아니면 플래그거나 배치거나." 지안은 스위치보드 콘솔을 열었다. 스위치보드는 회사가 쓰는 피처 플래그 시스템이었다. 프로덕션 환경의 변경 이력이 최근순으로 떴다.

> **reconcile-batch-v2** · OFF → ON · 02:11:52 · actor: **유도현**

지안은 그 줄을 두 번 읽었다. 유리문 밖에서 도현이 걸음을 멈췄다. 그도 같은 화면을 휴대폰으로 보고 있는 모양이었다.

"reconcile-batch-v2 플래그가 02:11에 켜졌어요." 유리가 말했다. "그거 샤드마다 커넥션 따로 여는 거라 풀 안 타요. 그게 원인 같은데요."

빨랐다. 지안이 화면을 보고 아직 입을 열기도 전이었다.
...
지안은 그 말이 사실이라는 걸 회사에서 유일하게 아는 사람이었다. 그리고 그걸 말하는 순간 무슨 일이 생기는지도 알았다. "02:11에 유도현은 내 맞은편에서 소주를 마시고 있었다"는 문장은 인시던트 채널에 적히는 즉시 다른 문장을 하나 더 만든다. 서지안은 소주 두 병을 마신 상태로 SEV1 인시던트 커맨더를 잡았다. 그 규정을 쓴 사람이 서지안이었다.
...
토요일 아침, 지안은 커피를 내리고 타임라인을 다시 처음부터 썼다. 인시던트 커맨더의 습관이었다. 사람의 기억은 새벽 두 시에 특히 부정확하고, 로그는 그렇지 않다.
...
지안은 자기 세션 기록을 확인했다. 그날 오후 두 시에 지안은 사내 발표 중이었다. 발표 녹화 영상에 타임스탬프가 남아 있었다.

세 번째는 IP였다. 10.40.0.7은 사무실 VPN 게이트웨이의 내부 주소였다. CI 러너는 VPN을 타지 않는다. 사람이 VPN에 접속해서 API를 직접 호출한 것이다. 새벽 두 시 십일 분에. VPN 접속 로그를 열자 그 시각에 연결되어 있던 사람은 셋이었다. 유리, 야근 중이던 데이터팀 인턴, 그리고 지안 자신. 도현의 이름은 없었다. 도현의 휴대폰은 그 시각에 VPN에 붙어 있지 않았다.
...
지안이 스위치보드 콘솔을 연 건 02:17:40이었다. 브리지 콜 녹취를 돌려보니 지안이 "배포가 아니면 플래그거나 배치거나"라고 말한 게 02:16:50이었다. 유리는 그 말이 끝나고 13초 뒤에 플래그 이름과 원인 메커니즘까지 정확히 말했다. 커넥션 풀 대시보드는 어떤 배치가 돌고 있는지 알려주지 않는다. 그걸 알려면 플래그 콘솔을 열거나, 배치 워커 로그를 열거나, 아니면 이미 알고 있어야 했다.

다섯 번째는 플래그를 끄는 데 걸린 시간이었다. 유리가 "하고 있어요"라고 말한 게 02:18. 실제로 꺼진 게 02:31. 13분. 스위치보드 콘솔 응답 시간 대시보드에서 그 시각의 p99는 400밀리초였다. 느리지 않았다.

지안은 배치 워커 로그를 열었다. reconcile-batch-v2는 02:12:04에 시작해서 02:29:47에 완료 상태로 끝났다. 플래그가 꺼지기 1분 13초 전이었다. 중간에 끊긴 게 아니라 끝까지 돈 것이다. 그 배치가 하는 일은 원본 결제 이벤트에서 정산 원장을 처음부터 다시 계산해서 덮어쓰는 것이었다. 정산 원장에 어떤 오류가 있었든, 배치가 끝난 뒤에는 남아 있지 않는다.
...
지안은 타임라인을 화면에 띄웠다. 로그를 하나씩 넘겼다. API 토큰. 비밀 저장소 접근 기록과 sre-admin 권한. VPN 접속자 명단. 13초. 13분. 배치 완료 시각. 412건. 8월 28일.

"블레임리스는 실수를 위한 원칙이에요. 사람은 실수하고, 실수한 사람을 벌주면 다음부터 숨기니까 벌주지 말자는 거죠. 근데 이건 처음부터 숨기기 위해 만든 장애예요. 8월 28일 마이그레이션에서 환불 412건이 날아갔고, 감사 전에 원장을 맞춰야 했고, 원장을 다시 계산하는 유일한 방법이 v2 배치였고, v2 배치는 아직 프로덕션에서 켜면 안 되는 물건이었어요. 그래서 새벽에 켰고, 켠 사람이 결제팀 토큰 이름으로 남게 했고, 배치가 끝날 때까지 안 끄고 기다렸어요."

"지안 씨." 민석이 말했다. "그건 추정이잖아요."

"비밀 저장소 세션 ID는 추정이 아니에요. 9월 8일 오후 두 시, 유리 씨 세션이에요. 그 시간에 저는 발표 중이었고 녹화가 있어요. 그리고 02:11에 VPN에 붙어 있던 사람 중에 도현 씨는 없어요."
